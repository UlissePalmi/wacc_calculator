...
def fetch_stock_data(ticker, benchmark='^GSPC', years=5):
...
def calculate_returns(prices, period='monthly'):
...
def calculate_beta_regression(stock_returns, market_returns):
    ...
    ss_res = np.sum((stock_r - predicted) ** 2)
    ss_tot = np.sum((stock_r - np.mean(stock_r)) ** 2)
    r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0
    
    n = len(aligned)
    se_beta = np.sqrt(ss_res / (n - 2)) / np.sqrt(np.sum((market_r - np.mean(market_r)) ** 2)) if n > 2 else 0
...
def unlever_beta(levered_beta, debt_equity_ratio, tax_rate):
...
def relever_beta(unlevered_beta, debt_equity_ratio, tax_rate):
    """Re-lever beta using Hamada equation."""
    leverage_factor = 1 + (1 - tax_rate) * debt_equity_ratio
    return unlevered_beta * leverage_factor


def generate_sample_returns(beta_target=1.0, n_months=60):
    """Generate sample return data."""
    np.random.seed(42)
    market_returns = np.random.normal(0.009, 0.045, n_months)
    stock_returns = 0.001 + beta_target * market_returns + np.random.normal(0, 0.03, n_months)
    dates = pd.date_range(end=datetime.now(), periods=n_months, freq='ME')
    return pd.Series(stock_returns, index=dates), pd.Series(market_returns, index=dates)
...
def main():
    ...
    print_header("ALLEGION (ALLE) BETA ANALYSIS WITH PEER COMPARISON")
    print(f"""
Company: {ALLEGION_DATA['company_name']} ({ALLEGION_DATA['ticker']})
Data Source: Form 10-K for Fiscal Year Ended {ALLEGION_DATA['fiscal_year_end']}
Filing Date: {ALLEGION_DATA['filing_date']}
    """)
    
    # =========================================================================
    # STEP 1: Display Allegion Financial Data
    # =========================================================================
    print_header("STEP 1: ALLEGION FINANCIAL DATA (FROM 10-K)")
    
    print(f"""
BALANCE SHEET (Page F-4):
  Total Debt:              ${ALLEGION_DATA['total_debt']:>10,.1f} million
  Total Equity:            ${ALLEGION_DATA['total_equity']:>10,.1f} million
  Cash & Equivalents:      ${ALLEGION_DATA['cash']:>10,.1f} million

CALCULATED RATIOS:
  Debt-to-Equity:          {ALLEGION_DATA['debt_to_equity']:>10.4f}
  Interest Coverage:       {ALLEGION_DATA['interest_coverage']:>10.2f}x

TAX (Note 18, Page F-27):
  Effective Tax Rate:      {ALLEGION_DATA['effective_tax_rate']*100:>10.1f}%
    """)
    
    # =========================================================================
    # STEP 2: Calculate Allegion's Regression Beta
    # =========================================================================
    print_header("STEP 2: ALLEGION REGRESSION BETA (5-YEAR MONTHLY)")
    
    print("\nFetching historical data for ALLE...")
    stock_data, market_data = fetch_stock_data('ALLE', '^GSPC', years=5)
    
    if stock_data is not None and len(stock_data) > 0:
        print("  ✓ Successfully fetched live data from Yahoo Finance")
        stock_returns = calculate_returns(stock_data, 'monthly')
        market_returns = calculate_returns(market_data, 'monthly')
    else:
        print("  Using simulated data...")
        stock_returns, market_returns = generate_sample_returns(beta_target=1.05)
    
    alle_regression = calculate_beta_regression(stock_returns, market_returns)
    alle_levered_beta = alle_regression['beta']
    alle_unlevered_beta = unlever_beta(alle_levered_beta, ALLEGION_DATA['debt_to_equity'], 
                                        ALLEGION_DATA['effective_tax_rate'])
    
    print(f"""
Regression Results:
  Levered Beta (β):        {alle_levered_beta:.4f}
  Alpha (α):               {alle_regression['alpha']*100:.4f}% monthly
  R-squared:               {alle_regression['r_squared']:.4f}
  Correlation:             {alle_regression['correlation']:.4f}
  Observations:            {alle_regression['n_observations']}

Unlevered Beta (Hamada):
  Formula: β_U = β_L / [1 + (1-T) × D/E]
  β_U = {alle_levered_beta:.4f} / [1 + (1-{ALLEGION_DATA['effective_tax_rate']:.3f}) × {ALLEGION_DATA['debt_to_equity']:.4f}]
  β_U = {alle_levered_beta:.4f} / {1 + (1-ALLEGION_DATA['effective_tax_rate']) * ALLEGION_DATA['debt_to_equity']:.4f}
  
  ┌─────────────────────────────────────────────┐
  │  Allegion Unlevered Beta: {alle_unlevered_beta:.4f}            │
  └─────────────────────────────────────────────┘
    """)
    ...
    _, market_data_peers = fetch_stock_data('^GSPC', '^GSPC', years=5)
    if market_data_peers is not None:
        market_returns_peers = calculate_returns(market_data_peers, 'monthly')
    else:
    ...
    for ticker, peer_info in PEER_COMPANIES.items():
        ...
        peer_stock_data, _ = fetch_stock_data(ticker, '^GSPC', years=5)
        
        if peer_stock_data is not None and len(peer_stock_data) > 0:
            peer_returns = calculate_returns(peer_stock_data, 'monthly')
...
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  METHOD 1: REGRESSION (5-year monthly returns)                           │
...
└──────────────────────────────────────────────────────────────────────────┘
    ...
    create_regression_chart(
        ...
        alle_regression,
        company_name='ALLE',
        output_path=f'{output_dir}/allegion_regression_chart.jpeg'
    )
